...
export function errorSimulationMiddleware(req, res, next) {
  ...
  if (req.path.startsWith('/auth') || req.path.startsWith('/debug') || req.path.startsWith('/health')) {
    return next();
  }
  ...
  if (shouldError) {
    ...
    console.log(`❌ [ERROR SIMULATION] ${req.method} ${req.path} → ${error.status} ${error.message}`);
