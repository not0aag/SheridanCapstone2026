...
router.post(
  ...
  (req, res) => {
    ...
    const activeTrips = list(
      "trips",
      (trip) => trip.userId === req.user.id && trip.status === "active"
    );
...
router.post(
  ...
  (req, res) => {
    ...
    const duration = Math.floor(
      (new Date(endTime) - new Date(trip.startTime)) / 1000
    );
    ...
    const deltaLat =
      ((endLocation.lat - trip.startLocation.lat) * Math.PI) / 180;
    const deltaLng =
      ((endLocation.lng - trip.startLocation.lng) * Math.PI) / 180;
    ...
    // Calculate speeds
    const averageSpeed =
      distance > 0 ? Math.floor((distance / duration) * 3.6) : 0; // m/s to km/h
    const maxSpeed =
      averageSpeed > 0
        ? Math.floor(averageSpeed * (1.2 + Math.random() * 0.3))
        : 0;
...
router.get(
  ...
  (req, res) => {
    ...
    if (startDate) {
      trips = trips.filter(
        (trip) => new Date(trip.startTime) >= new Date(startDate)
      );
    }
    if (endDate) {
      trips = trips.filter(
        (trip) => new Date(trip.startTime) <= new Date(endDate)
      );
    }
