...
app.get('/', (req, res) => {
  res.json({
    ...
    documentation: 'https://github.com/not0aag/SheridanCapstone2026/blob/main/tools/mock-server/README.md',
    endpoints: {
      auth: {
    ...
    config: {
      ...
      errorSimulation: config.enableErrorSimulation ? `enabled (${config.errorRate * 100}% rate)` : 'disabled'
    }
  });
});
...
async function startServer() {
  try {
    ...
    app.listen(config.port, () => {
      ...
      console.log(`   Error Simulation: ${config.enableErrorSimulation ? 'ENABLED' : 'disabled'}`);
