...
export async function seedData() {
    ...
    const trip = create("trips", {
      ...
      distance:
        status === "completed"
          ...
          : null,
      averageSpeed:
        status === "completed" ? Math.floor(Math.random() * 50) + 40 : null,
      maxSpeed:
        status === "completed" ? Math.floor(Math.random() * 40) + 80 : null,
      startLocation: {
    ...
    if (status === "completed") {
      ...
      for (let j = 0; j < numIncidents; j++) {
        ...
        const incidentTime = new Date(
          startTime.getTime() + Math.random() * duration * 1000
        );
  ...
  console.log(
    `✅ Seeded ${db.users.size} users, ${db.trips.size} trips, ${db.incidents.size} incidents, ${db.videos.size} videos`
  );
}
