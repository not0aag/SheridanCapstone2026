"""
Test Video with Hair/Makeup Bias Fix
- Suppresses c8 (hair/makeup) predictions unless very confident
- Boosts c0 (safe driving) detection
- Only alerts on real distractions
"""
...
DISTRACTION_ALERT_THRESHOLD = 75.0   # Higher threshold for distractions
SAFE_DRIVING_THRESHOLD = 45.0        # Lower threshold for safe driving
HAIR_MAKEUP_THRESHOLD = 85.0         # Much higher threshold for hair/makeup (heavily suppress)
...
class VideoAnalyzer:
    ...
    def predict(self, frame):
        """Get prediction with bias correction"""
        input_data = self.preprocess_frame(frame)
        self.interpreter.set_tensor(self.input_details[0]['index'], input_data)
        self.interpreter.invoke()
        output_data = self.interpreter.get_tensor(self.output_details[0]['index'])[0]

        # BIAS CORRECTION: Suppress hair/makeup
        # Reduce c8 confidence by 30% to counteract model bias
        output_data[8] = output_data[8] * 0.70

        # BIAS CORRECTION: Boost safe driving slightly
        # Increase c0 confidence by 15% to help detection
        ...
        # Get top prediction after bias correction
        ...
        # ENHANCED CONFIDENCE-BASED DECISION LOGIC:

        # 1. SPECIAL CASE: If top prediction is hair/makeup, require VERY high confidence
        if top_class == 8:
            if top_confidence >= HAIR_MAKEUP_THRESHOLD:
                # Only accept if VERY confident
                return 8, top_confidence, "ALERT", output_data
            else:
                # Otherwise, check if safe driving is reasonable alternative
                if c0_confidence >= 35.0:
                    return 0, c0_confidence, "SAFE (UNCERTAIN)", output_data
                else:
                    # Find second-best prediction
                    second_idx = np.argsort(output_data)[-2]
                    second_conf = output_data[second_idx] * 100
                    if second_idx == 0 or second_conf < 30.0:
                        return 0, c0_confidence, "SAFE (DEFAULT)", output_data
                    else:
                        return second_idx, second_conf, "UNCERTAIN", output_data

        # 2. If c0 is above threshold, assume safe driving
        if c0_confidence >= SAFE_DRIVING_THRESHOLD:
            return 0, c0_confidence, "SAFE", output_data

        # 3. If distraction is detected with HIGH confidence (and not c8), alert
        if top_class != 0 and top_class != 8 and top_confidence >= DISTRACTION_ALERT_THRESHOLD:
            return top_class, top_confidence, "ALERT", output_data

        # 4. If no strong signal, default to safe driving
        if c0_confidence >= 30.0:
            return 0, c0_confidence, "SAFE (LOW CONF)", output_data

        # 5. Otherwise, show top prediction but mark as uncertain
    ...
    def draw_overlay(self, frame, pred_class, confidence, status, all_confidences, fps, frame_num, total_frames):
        ...
        cv2.putText(frame, "SafeDrive AI - Bias-Corrected Analysis", (20, 35),
        ...
        cv2.putText(frame, "Top Predictions (Bias-Corrected):", (20, bottom_y + 30),
        ...
        for i, idx in enumerate(top3_idx):
            ...
            if idx == 0:
                ...
                color = (100, 100, 100)  # Gray for hair/makeup (suppressed)
            else:
                color = (100, 100, 255) if conf >= DISTRACTION_ALERT_THRESHOLD else (150, 150, 150)

            # Add suppression indicator for c8
            suffix = " (SUPPRESSED)" if idx == 8 and conf < HAIR_MAKEUP_THRESHOLD else ""
            text = f"{i+1}. {name}: {conf:.1f}%{suffix}"
            cv2.putText(frame, text, (20, y_pos),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 1)
            y_pos += 30

        # Bias correction info
        cv2.putText(frame, "c8 suppressed -30% | c0 boosted +15%",
                   (20, height - 40),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (100, 100, 100), 1)

        # FPS and threshold info
        cv2.putText(frame, f"FPS: {fps:.1f} | c8 threshold: {HAIR_MAKEUP_THRESHOLD:.0f}% | Other: {DISTRACTION_ALERT_THRESHOLD:.0f}%",
                   (20, height - 10),
    ...
    def analyze_video(self, video_path, save_output=False):
        """Analyze video file with bias correction"""
        ...
        print(f"  c8 (Hair/Makeup) confidence reduced by 30%")
        print(f"  c0 (Safe Driving) confidence boosted by 15%")
        print(f"  c8 alert threshold: >= {HAIR_MAKEUP_THRESHOLD}% (very high)")
        print(f"  Other distractions: >= {DISTRACTION_ALERT_THRESHOLD}%")
        print(f"  Safe driving: >= {SAFE_DRIVING_THRESHOLD}%")
        ...
        out = None
        if save_output:
            output_path = video_path.parent / f"{video_path.stem}_fixed.mp4"
        ...
        while True:
            ...
            # Display
            cv2.imshow('SafeDrive AI - Bias-Corrected Analysis', frame_display)
        ...
        for class_id, count in sorted(stats['class_counts'].items(), key=lambda x: x[1], reverse=True):
            if count > 0:
                ...
                suppressed = " (HEAVILY SUPPRESSED)" if class_id == 8 else ""
                print(f"  {marker} c{class_id} ({CLASS_NAMES[class_id]:30}): {count:5} frames ({pct:5.1f}%){suppressed}")
...
def main():
    print("\n" + "="*80)
    print("SafeDrive AI - Bias-Corrected Video Analysis")
    print("="*80 + "\n")
    print("FIXES: Hair/Makeup bias by suppressing c8 predictions")
    print("       Boosts Safe Driving (c0) detection\n")
