...
class AndroidDecisionEngineTest {
    ...
    fun `no alert when eyes open and not distracted`() {
        repeat(5) { engine.addFrame(eyesClosed = false, isDistracted = false) }
    ...
    fun `sustained distraction above the default rate threshold triggers DISTRACTED`() {
        // 25-frame window, all distracted -> rate 1.0, above the default 0.40 threshold.
        // DISTRACTED does not require the elapsed-time gate DROWSY does, so no sleep needed.
        repeat(25) { engine.addFrame(eyesClosed = false, isDistracted = true) }
        val decision = engine.getDecision()
    ...
    fun `distraction rate below the default threshold does not trigger an alert`() {
        ...
        repeat(20) { engine.addFrame(eyesClosed = false, isDistracted = false) }
        repeat(5) { engine.addFrame(eyesClosed = false, isDistracted = true) }
        val decision = engine.getDecision()
        assertEquals("", decision.alertType)
    }
    ...
    fun `lowering distRateThreshold makes the same distraction rate fire an alert`() {
        // Same 0.2 rate as the "below threshold" case above, but with a lowered
        // threshold — regression test for the Settings sensitivity slider wiring.
    ...
    fun `sustained eye closure past the drowsy window triggers DROWSY`() {
        ...
        engine.addFrame(eyesClosed = true, isDistracted = false)
        Thread.sleep(4100)
        // Frames older than the 4s window get pruned, so refresh it post-sleep.
        repeat(3) { engine.addFrame(eyesClosed = true, isDistracted = false) }
